...
//=============================================================================
void set_banks()			// мало того что у каждой машины куча if MM_ATM3
{					// так оно потом вызывает еще одну универсальную городуху
					// с еще одной тучей if MM_ATM3
// input:
//	ports	7FFD
//		1FFD
//		DFFD
...
*/
	...
	case MM_PLUS3:
	{
		...
		// а устоновить bakn1, bank2 ???
		// после 4-х оконного режима
		...
		bankw[1] = bankr[1] = RAM_BASE_M + PAGE * RamDecoder[RamPage][1];	// ибо нехуй
	...
	case MM_QUORUM:
	{
		...
		comp.flags |= CF_DOSPORTS;	// чота включает SHADOW MODE
	    //-----------------------------------------------------------------
	    if (comp.p00 & Q_B_ROM)
	    {
		if (comp.flags & CF_TRDOS)
