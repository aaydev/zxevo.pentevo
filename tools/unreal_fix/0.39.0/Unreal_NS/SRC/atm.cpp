...
{
    // такое шуства, что это гафно перекидывает 2 метра памяти
    // а не перепутывает сраные биты
    //    потом будет куча багов с дебагером, бряками итд...
    //-------------------------------------------------------------------------
...
//	printf("%d->%d SKIPPED!\n",  OldVideoMode, NewVideoMode);
    ...
    //                 слева и справа от растра: 32 такта (64 пикселя).
    // 
    ...
    //     - Если луч на бордюре сверху от растра или на бордюре слева от растра
    //         - изменяем оффсет для текущей сканлинии
    //     - Если луч на растре или на бордюре справа от растра
    //         - изменяем оффсет для следующей сканлинии
    //-------------------------------------------------------------------------
...
void set_atm_FF77(unsigned port, unsigned char val)
{
    //-------------------------------------------------------------------------
    if ((comp.pFF77 ^ val) & 1)
	atm_memswap();	// атээмо бага swap memory address bits A5-A7 and A8-A10
    //-------------------------------------------------------------------------
...
void set_atm_aFE(unsigned char addr)
{
    ...
    comp.aFE = addr;
    //-------------------------------------------------------------------------
    if ((addr ^ old_aFE) & 0x40) atm_memswap();	// атээмо бага swap memory address bits A5-A7 and A8-A10
...
unsigned char atm620_z(unsigned t)		//[NS]
{
    // для запуска MSX-DOS
    //-------------------------------------------------------------------------
    if (	((unsigned)(t) >= 0)	&&		// предположение на основе кода проверки
		((unsigned)(t) < 1024)			// возможно там вообще меандр
	)
    {
	//printf("t %d = 0\n",t);
	return 0;
