...
static void ay_led()
{
   aypos = temp.led.ay;
   unsigned char sum=0;
				//поменять цвета чтоб не конфликтовали с палитрой!
//tone 		белый
//шум		желтый
//огибающая	зеленый
   int max_ay = (conf.sound.ay_scheme > AY_SCHEME_PSEUDO)? 2 : 1;
   for (int n_ay = 0; n_ay < max_ay; n_ay++) 	//AY-ки
   {
	 ...
	 //tone
	 if (!tone) c1 = c2 = 0x0F;	//белый
	 
	 //noise
         if (!noise) c2 = 0x0E;		//желтый
	 

	if (v & 0x10) 
	{
	    //envelope
	    //видимо тут прямое чтение огибающе
	...
	else 
	{
		...
		v &= 0x0F;	//белый
		//c1 = 0x03;	//тест
				//наверное от сюда можно пофиксить
				//старый вариант цвета шума
	}
...
static void show_highres_256_mband( unsigned char *dst)
{
	...
	//---------------------------------------------------------------------
	//полосочки отделяющие 16К по бокам
	...
	for (unsigned p = 0;    p < 32;    p++, dst += 2) 
	{
	    ...
	    // запись байт + цвет (4.4 paper ink)
...
static void show_mband(unsigned char *dst, unsigned start)
{
    ...
    //printf("memband---------------------\n");
    //тут работает сразу и каждый фрейм
	...
	//---------------------------------------------------------------------
	band[ i] = res;	//сборка "усредненных" 128 значений
    }
    //-------------------------------------------------------------------------
	...
	dst[ 0 * pitch] = t;	dst[ 0 * pitch + 1] = 0x9B;	//рамка ?
	...
	dst[ 7 * pitch] = t;	dst[ 7 * pitch + 1] = 0x9B;	//рамка ?
    }
    //-------------------------------------------------------------------------
    sprintf( xx, "%02X", (start - 1) >> 8);
    text_i( dst, xx, 0x0B, 2);			//цифры справа    
    //-------------------------------------------------------------------------
    for (i = 0;    i < 8;    i++)
    {
	dst[ i * pitch] |= 0x80;
...
static void memband_led()
{
	...
	dst += 7 * pitch;			// +10*pitch; в оригинале
    }
    //-------------------------------------------------------------------------
    Z80 &cpu = CpuMgr.Cpu();
    //-------------------------------------------------------------------------
...
void showleds()
{
    ...
    // dbgchk - Признак наличия активных брекпоинтов
	...
	{	
	    memband_led();
	    cpu.dbgchk |= 1; 	// для force memory band [NS]
	}
	//---------------------------------------------------------------------
