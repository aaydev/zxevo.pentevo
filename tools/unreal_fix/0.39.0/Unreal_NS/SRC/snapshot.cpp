...
int loadsnap(char *filename)
{
	...
	if (ok)
	{
		...
		*p = 0;
	    ...
	    strcat(wintitle," - Unreal_NS");	//для загружаемого дисков
...
/*  if (conf.ramsize != 128) 
    ...
    //-------------------------------------------------------------------------
    if (comp.p7FFD == 0x30)	//но для пентагона это не значет 48 lock !!!
    { // save 48k
	mapped = 0xFF;
	savesize = 0xC01B;
	hdr->sp -= 2;
...
static int readPAL()
{
    ...
    if (conf.ula_plus)
    {
	// Если присутствует ULA+, то сразу и активируем ее (чтобы изменения палитры были видны)
...
again:
	...
	*path = 0; // delete extension
	...
	if (ff)
	{
		...
		strcpy(wintitle,name);
//		strcat(wintitle," - UnrealSpeccy");
		strcat(wintitle," - Unreal_NS");	// для сохраненного диска
		SetWindowText(wnd, wintitle);
		//~---------Alone Coder
	    }
	    //-----------------------------------------------------------------
...
//  static unsigned sshot = 999990;//0;	// стартовый номер скриншота
    ...
    //-------------------------------------------------------------------------
    // Первая проверка имени файла (когда пустая папка или номер последнего файла уже известен) [NS] r0110
    // printf("%X\n",sshot);
    snprintf(	fname,
		_countof(fname),
    ...
    //-------------------------------------------------------------------------
    // если таки такого файла таки нет то молча таки сохраняем	[NS] r0110
    if (access (fname, 0) != 0)
    {
	goto screenshot_name_valid;
    }
    //-------------------------------------------------------------------------
    // letz brute new name тогда				[NS] r0110
    // но тк брутить 9 сен манов файлов это не камильфо
    // а как правильно получить номер последнего файла ЯХЗ
    // то
    //
    { // просто ограничение мерской видимости для нормального джампанья на screenshot_name_valid
	sshot = 0xFFFFFFFF;
	...
	//---------------------------------------------------------------------
	// сбрасываем каждый бит номера числа от d31 до d0
	// и сверяем есть ли такой файл
	while (temp_cnt < 32)
	{
	    sshot ^= roll_bit;
	    //printf("%X - ",sshot);
