...
//   {
//       //Разрешена работа с ОЗУ шрифта для ATM3
...
// Read Breakpoint ???
    ...
    *membit |= MEMBITS_R;	// memoryband
    ...
    if ((*bp_disable_bit & BP_DISABLE_BPR) == 0)	// не оптимально !!!!! [NS]
    {
...
// Write Breakpoint ?????????
    ...
    *membit |= MEMBITS_W;	// memoryband
    ...
    // и инверсировать назначение битов в BP_DISABLE_XXX !!!!!
    if ((*bp_disable_bit & BP_DISABLE_BPW) == 0)	// не оптимально !!!!! [NS]
    {
...
//	unsigned idx = (addr >> 3) | ((addr & 7) << 8);			//0.39.0
	...
	update_screen();
//	return;		//ЕСТЬ В 0.39.0
       //return;	//ЗАКОМЕНЧЕННО В NEDOREPO
   }
//-----------------------------------------------------------------------------
// ЗАКОМЕНЧЕННАЯ СТРОКА ИЗ NEDOREPO
...
{
    unsigned char temp_op_code;		// КАКАЯ ТО МУТЬ ИЗ
    ...
    {
	// Примитивная имитация MIDI Out-а фирменного 128-го
	//---------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
    ...
    cpu->t += 4;
//-----------------------------------------------------------------------------
//  return xm(cpu->pc++);	//ЗАКОМЕНТИЛИ В NEDOREPO
				//ЕСТЬ В 0.39.0
//-----------------------------------------------------------------------------
...
//NEDOREPO - ашо в пентеве инт нельзя отключить как в АТМ?
    if ( cpu.int_pend && cpu.iff1 && cpu.t != cpu.eipos && // int enabled in CPU not issued after EI
...
// . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 
#ifdef Z80_DBG
      debug_events(&cpu);		//там же вызов дебагера
#endif
// . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 
      step();
