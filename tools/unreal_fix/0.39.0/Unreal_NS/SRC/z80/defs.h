...
struct TZ80State
{
	...
	struct
	{
	    ...
	    unsigned char iff1;		// int = 1 -> EI
					// int = 0 -> DI
	    unsigned char iff2;		// состояние int на момент NMI
    ...
    union
    {
	unsigned bc;			//это шо за муть ??????!!!!!
	unsigned short bc16;		//оно ссылаетсо на одно и тоже? [NS]
	struct
	{
	    unsigned char c;
	    unsigned char b;
...
struct Z80 : public TZ80State
{
    ...
    //-------------------------------------------------------------------------
    // Trace Window
    unsigned	trace_curs;		// адрес курсора
    unsigned	trace_top;		// адрес самой верхней строки
    int		graph_trace_cursor_pos;	// чисто графическое положение курсора [NS]
    unsigned	trace_mode;		// 0 - addr
					// 1 - dump/labels
					// 2 - disasm
    //-------------------------------------------------------------------------
    // Memory Viewer
    unsigned mem_curs;			// адрес курсора
    unsigned mem_top;			// адрес начала самой верхней видимой строки
    unsigned mem_second;		// режим редактирования второго нибла
    unsigned mem_addr_edit_mode;	// режим редактирования адреса [NS]
    //-------------------------------------------------------------------------
    unsigned pc_trflags;
    unsigned nextpc;
    //-------------------------------------------------------------------------
    ...
    bool int_gate;		// Разрешение внешних прерываний
				//     1 - разрешены
				//     0 - запрещены
    //-------------------------------------------------------------------------
    #define MAX_CBP 16
...
//  TMemDbg DirectMem;		// get direct memory pointer in debuger
   ...
   //-------------------------------------------------------------------------
    ...
    //установка числа тактов в фрейме
