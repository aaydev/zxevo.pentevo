...
//	main_pause_flag = 0;	//NS
    ...
    conf.win_resize_request = 1;	// начальная установка размера [NS]
    conf.win_static_size_x = 777;
    conf.win_static_size_y = 555;
	
    conf.trace_follow_regs = REG_PC;	// следить в дебагере за PC по дефолту [NS]
    conf.trace_follow_request = 1;	// и сигнализируем что надо бы этот PC показать
    
    // на всякий случай инициализируем hwnd-ы не модальных окон
    hwnd_bp_dialog = NULL;	// hwnd немодального окна бряков [NS]
    hwnd_watchdlg = NULL;	// hwnd немодального окна вачесов [NS]
    ...
    //-------------------------------------------------------------------------
    init_z80tables();
    video_permanent_tables();	//генерация таблиц для видео?
    init_ie_help();
    load_config( config);
    //-------------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------------
//  make_samples();	//те самые выпеленные ay samples
    init_leds();
    init_tape();
    init_hdd_cd();
    start_dx();		// создает главное окно
    set_priority();
    applyconfig();
    main_reset();
    autoload();
    ...
    {
	midi_init();		// пока включаетсо только из конфига
    }
    //-------------------------------------------------------------------------
//  if (conf.wiznet)	//ЗАКОМЕНЧЕНО В САМОМ NEDOREPO
//  { 		
//	Wiz5300_Init(); 
//  } 
    //-------------------------------------------------------------------------
