...
#include "dxr_pentatm7.h"	//NEDOREPO
...
//  if ( 7 == (comp.pFF77 & 7) ) // Undocumented Sinclair Textmode VideoMode
    if ( (7 == (comp.pFF77 & 7)) && (conf.mem_model!=MM_ATM3) ) 
    //< Undocumented Sinclair Textmode VideoMode --lvd-- in ATM2	//NEDOREPO
    {
        rend_atm7(dst, pitch);
        return;
    }
...
//-----------------------------------------------------------------------------
    if ( (7 == (comp.pFF77 & 7)) && (conf.mem_model==MM_ATM3) ) 
    // lvd: pentevo one-page textmode		//NEDOREPO
    {
		for(int y=0; y<200; y++)
